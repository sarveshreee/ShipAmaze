...
export const listWeightDisputes = asyncHandler(async (req: AuthRequest, res: Response) => {
  if (!req.user) throw new AppError(401, "Unauthorized");

  const filter: Record<string, unknown> = {};
  if (req.user.role !== "admin") {
    const { buildOrderVisibilityQuery } = await import("../utils/orderFilters.js");
    const visibility = await buildOrderVisibilityQuery(req.user);
    const owned = await Order.find(visibility).select("orderId awb").lean();
    const orderIds = owned.map((o) => String(o.orderId ?? "").trim()).filter(Boolean);
    const awbs = owned.map((o) => String(o.awb ?? "").trim()).filter(Boolean);
    if (orderIds.length === 0 && awbs.length === 0) {
      res.json([]);
      return;
    }
    filter.$or = [
  ...
  const rows = await WeightDispute.find(filter).sort({ createdAt: -1 }).limit(500).lean();
  res.json(
    rows.map((w) => ({
      id: w.disputeId,
      orderId: w.orderId,
