...
import {
  Shield, Building2, Truck, Users2, Upload, CheckCircle2, Clock, AlertCircle, Plus,
  Star, Trash2, Copy, RefreshCw, User, Briefcase, FileCheck, X
} from "lucide-react";
...
function KycTab({ userId, isDemoMode }: { userId: string | null; isDemoMode: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      if (isDemoMode || !userId) {
        const cached = localStorage.getItem(LS.kyc);
        if (cached) setProfile(JSON.parse(cached));
        setLoading(false);
        return;
      }
      const { data } = await supabase.from("kyc_profiles").select("*").eq("user_id", userId).maybeSingle();
      if (data) setProfile({ ...data, uploaded_docs: (data.uploaded_docs as any) || {} } as any);
      setLoading(false);
    })();
  }, [userId, isDemoMode]);
  ...
  const persist = async (next: KycProfile) => {
    if (isDemoMode || !userId) {
      localStorage.setItem(LS.kyc, JSON.stringify(next));
      return;
    }
    const payload: any = { ...next, user_id: userId, uploaded_docs: next.uploaded_docs || {} };
    delete payload.id;
    const { error } = await supabase.from("kyc_profiles").upsert(payload, { onConflict: "user_id" });
    if (error) throw error;
  };

  const saveDraft = async () => {
    try { await persist({ ...profile, status: profile.status === "verified" ? "verified" : "draft" }); toast.success("Draft saved"); }
      ...
      <div className="rounded-xl bg-card shadow-card p-6 space-y-6">
        ...
        <fieldset disabled={isLocked} className="space-y-4 disabled:opacity-70">
          {profile.account_type === "individual" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div><Label>Date of birth</Label><Input type="date" value={profile.dob || ""} onChange={e => update({ dob: e.target.value })} /></div>
              ...
              <FileUploadField label="Aadhaar (front & back)" value={profile.uploaded_docs?.aadhaar} onChange={v => updateDoc("aadhaar", v)} />
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-border">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={saveDraft} disabled={isLocked}>Save draft</Button>
            <Button onClick={submit} disabled={isLocked} className="bg-primary text-primary-foreground hover:bg-primary-dark">
...
/* ===========================================================
   BANK TAB
=========================================================== */
function BankTab({ userId, isDemoMode }: { userId: string | null; isDemoMode: boolean }) {
  ...
  const load = async () => {
    if (isDemoMode || !userId) {
      const cached = localStorage.getItem(LS.banks);
  ...
  const persistDemo = (list: BankAccount[]) => localStorage.setItem(LS.banks, JSON.stringify(list));

  const addAccount = async (a: Omit<BankAccount, "id" | "created_at" | "status" | "is_primary">) => {
    ...
    if (isDemoMode || !userId) {
      const next = [newAcct, ...accounts];
      setAccounts(next); persistDemo(next);
    } else {
      const { error } = await supabase.from("bank_accounts").insert({
        ...
        status: "verifying",
        is_primary: newAcct.is_primary,
      });
      if (error) { toast.error(error.message); return; }
    }

    toast.success("Account added — penny-drop in progress");
    // Simulate penny-drop verification
    setTimeout(async () => {
      const verified = Math.random() > 0.15;
      if (isDemoMode || !userId) {
        setAccounts(prev => {
          const next = prev.map(x => x.id === newAcct.id ? { ...x, status: (verified ? "verified" : "failed") as BankStatus, } : x);
          persistDemo(next);
          return next;
        });
      } else {
      ...
      toast[verified ? "success" : "error"](verified ? "Bank account verified" : "Verification failed — please retry");
    }, 2500);
  };

  const setPrimary = async (id: string) => {
    const target = accounts.find(a => a.id === id);
    if (!target || target.status !== "verified") { toast.error("Only verified accounts can be primary"); return; }

    if (isDemoMode || !userId) {
      const next = accounts.map(a => ({ ...a, is_primary: a.id === id }));
      setAccounts(next); persistDemo(next);
    } else {
  ...
  const retry = async (id: string) => {
    if (isDemoMode || !userId) {
      setAccounts(prev => { const n = prev.map(a => a.id === id ? { ...a, status: "verifying" as BankStatus } : a); persistDemo(n); return n; });
    } else {
      await supabase.from("bank_accounts").update({ status: "verifying" }).eq("id", id);
      load();
    }
    toast.info("Retrying verification…");
    setTimeout(async () => {
      if (isDemoMode || !userId) {
        setAccounts(prev => { const n = prev.map(a => a.id === id ? { ...a, status: "verified" as BankStatus } : a); persistDemo(n); return n; });
      } else {
        await supabase.from("bank_accounts").update({ status: "verified", verified_at: new Date().toISOString() }).eq("id", id);
        load();
      }
...
function AddBankModal({ open, onClose, onAdd }: {
  ...
  onAdd: (a: { account_holder_name: string; account_number_masked: string; ifsc: string; bank_name: string; account_type: string }) => void;
}) {
  ...
  const submit = () => {
    ...
    onAdd({
      ...
      account_number_masked: maskAcct(acct),
      ifsc: ifsc.toUpperCase(),
      bank_name: bank.trim(),
      account_type: type,
    });
    setTimeout(() => { reset(); onClose(); }, 100);
  };
...
/* ===========================================================
   ROUTING TAB
=========================================================== */
function RoutingTab({ userId, isDemoMode }: { userId: string | null; isDemoMode: boolean }) {
  const [settings, setSettings] = useState<RoutingSettings>({ auto_route_enabled: false, default_vendor_id: null, default_vendor_name: null });
  const [vendors, setVendors] = useState<{ id: string; name: string }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      // Load vendors (from products table — distinct vendors)
      if (!isDemoMode && userId) {
        const { data: prods } = await supabase.from("products").select("vendor_id, vendor_name").not("vendor_id", "is", null);
        const map = new Map<string, string>();
        (prods || []).forEach((p: any) => { if (p.vendor_id && p.vendor_name) map.set(p.vendor_id, p.vendor_name); });
        const list = Array.from(map.entries()).map(([id, name]) => ({ id, name }));
        // Fallback demo vendors if empty
        setVendors(list.length ? list : [
          { id: "v1", name: "Mumbai Central Hub" },
          { id: "v2", name: "Delhi Warehouse" },
          { id: "v3", name: "Bangalore Fulfillment" },
        ]);

        const { data } = await supabase.from("routing_settings").select("*").eq("user_id", userId).maybeSingle();
        ...
        const cached = localStorage.getItem(LS.routing);
  ...
  const save = async () => {
    if (settings.auto_route_enabled && !settings.default_vendor_id) {
      toast.error("Please select a default vendor");
      return;
    }
    if (isDemoMode || !userId) {
      localStorage.setItem(LS.routing, JSON.stringify(settings));
    } else {
      const { error } = await supabase.from("routing_settings").upsert({
        user_id: userId,
        auto_route_enabled: settings.auto_route_enabled,
        default_vendor_id: settings.default_vendor_id,
        default_vendor_name: settings.default_vendor_name,
      }, { onConflict: "user_id" });
      if (error) { toast.error(error.message); return; }
    }
  ...
  return (
    <div className="rounded-xl bg-card shadow-card p-6 max-w-2xl">
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          <h3 className="text-lg font-semibold text-text-primary">Vendor Auto-Routing</h3>
          <p className="text-sm text-text-muted mt-0.5">Apne Shopify orders ko apne aap kisi vendor ko forward karwaein — unhe pack aur ship karne ke liye.</p>
        </div>
      </div>

      <div className="rounded-xl border border-border p-4 flex items-start justify-between gap-4 mb-5">
        <div>
          <p className="font-medium text-text-primary">Auto-route new orders</p>
          <p className="text-xs text-text-muted mt-1">Jab Shopify pe naya order aaye, system khud usko vendor ke dashboard pe bhej dega.</p>
        </div>
        <Switch checked={settings.auto_route_enabled} onCheckedChange={v => setSettings(s => ({ ...s, auto_route_enabled: v }))} />
      </div>

      <div className="space-y-2 mb-5">
        <Label>Default Vendor</Label>
        <Select
          value={settings.default_vendor_id || ""}
          onValueChange={v => {
            const found = vendors.find(x => x.id === v);
            setSettings(s => ({ ...s, default_vendor_id: v, default_vendor_name: found?.name || null }));
          }}
          disabled={!settings.auto_route_enabled}
        >
          <SelectTrigger><SelectValue placeholder="Vendor select karein" /></SelectTrigger>
          <SelectContent>
            {vendors.map(v => <SelectItem key={v.id} value={v.id}>{v.name}</SelectItem>)}
          </SelectContent>
        </Select>
        <p className="text-xs text-text-muted">Jin products ka specific vendor mapping nahi hai, wo orders is vendor ko jayenge.</p>
      </div>

      <div className="flex justify-end">
        <Button onClick={save} className="bg-primary text-primary-foreground hover:bg-primary-dark">Save Settings</Button>
      </div>
    </div>
  );
}
...
/* ===========================================================
   TEAM TAB
=========================================================== */
function TeamTab({ userId, isDemoMode }: { userId: string | null; isDemoMode: boolean }) {
  ...
  const load = async () => {
    if (isDemoMode || !userId) {
      const cached = localStorage.getItem(LS.team);
      setMembers(cached ? JSON.parse(cached) : []);
      setLoading(false);
      return;
    }
    const { data } = await supabase.from("team_members").select("*").eq("owner_user_id", userId).order("invited_at", { ascending: false });
  ...
  const persistDemo = (l: TeamMember[]) => localStorage.setItem(LS.team, JSON.stringify(l));
  ...
  const addMember = async (m: Omit<TeamMember, "id" | "invited_at" | "status">) => {
    ...
    const newM: TeamMember = { ...m, id: crypto.randomUUID(), invited_at: new Date().toISOString(), status: "invited" };
    if (isDemoMode || !userId) {
      const next = [newM, ...members]; setMembers(next); persistDemo(next);
    } else {
      const { error } = await supabase.from("team_members").insert({
        owner_user_id: userId, full_name: m.full_name, email: m.email, role: m.role, permissions: m.permissions, status: "invited",
      });
      if (error) { toast.error(error.message); return; }
      load();
    }
  ...
  const remove = async (id: string) => {
    ...
    if (isDemoMode || !userId) {
      const next = members.filter(m => m.id !== id); setMembers(next); persistDemo(next);
    } else {
  ...
  const resend = async (id: string) => {
    ...
    if (isDemoMode || !userId) {
      const next = members.map(m => m.id === id ? { ...m, invited_at: now } : m);
      setMembers(next); persistDemo(next);
    } else {
      await supabase.from("team_members").update({ last_resent_at: now }).eq("id", id);
      load();
    }
...
function InviteModal({ open, onClose, onAdd }: {
  open: boolean; onClose: () => void;
  onAdd: (m: { full_name: string; email: string; role: string; permissions: string[] }) => void;
}) {
  ...
  const submit = () => {
    ...
    onAdd({ full_name: name.trim(), email: email.trim().toLowerCase(), role, permissions: perms });
    reset(); onClose();
  };
