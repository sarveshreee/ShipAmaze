...
import { rateCardData, pincodeServiceData } from "@/data/mockData";
...
export default function DropshipperRates() {
  ...
  const [pinResult, setPinResult] = useState<typeof pincodeServiceData[0] | null>(null);

  const checkPin = () => {
    const found = pincodeServiceData.find(p => p.pincode === pincode);
    setPinResult(found || null);
  };
    ...
    <div className="animate-fade-in-up">
      ...
      <div className="flex gap-1 border-b border-border mb-6">
        {([
          ...
          { id: 'rateCard', label: 'Rate Card', icon: Truck },
          { id: 'pincheck', label: 'Pincode Check', icon: MapPin },
        ] as const).map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={cn("flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-[1px] transition-colors",
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  {['Prepaid', 'COD'].map(t => (
                    <button key={t} className={cn("flex-1 rounded-lg py-2 text-sm font-medium border-2 transition-all",
                      t === 'Prepaid' ? "border-primary bg-primary-light text-primary" : "border-border text-text-secondary"
                    )}>{t}</button>
                  ))}
                </div>
              </div>
              <Button onClick={() => setShowResults(true)} className="w-full bg-primary text-primary-foreground hover:bg-primary-dark">
                ...
                {couriersResult.map(c => (
                  <div key={c.name} className="p-4 flex items-center gap-4 hover:bg-surface-2/30 transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-text-muted">{c.mode} · {c.delivery}</p>
                    </div>
        ...
        <div className="rounded-lg bg-card shadow-card overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border bg-surface-2/50">
              ...
              {rateCardData.weightSlabs.map(w => <th key={w} className="p-3 text-center font-medium text-text-secondary">{w}</th>)}
            </tr></thead>
            <tbody>
              {rateCardData.zones.map((z, i) => (
                <tr key={z} className={cn("border-b border-border", i % 2 === 0 && "bg-surface-2/30")}>
                  ...
                  {rateCardData.rates[z].map((rate, j) => (
                    <td key={j} className="p-3 text-center text-text-primary">₹{rate}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        <div className="max-w-lg">
          <div className="rounded-lg bg-card shadow-card p-6">
            <h3 className="font-semibold text-text-primary mb-4 flex items-center gap-2">
              <MapPin className="h-5 w-5 text-primary" />Pincode Serviceability Check
            </h3>
            <div className="flex gap-2">
              <Input value={pincode} onChange={e => setPincode(e.target.value)} placeholder="Enter 6-digit pincode" className="flex-1" maxLength={6} />
              <Button onClick={checkPin} className="bg-primary text-primary-foreground hover:bg-primary-dark">
            ...
            {pinResult && (
              <div className="mt-4 rounded-lg bg-success-light p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-success-dark">Serviceable — {pinResult.city}, {pinResult.state}</span>
                </div>
                <p className="text-sm text-text-secondary">Zone {pinResult.zone} · {pinResult.couriers.length} couriers available</p>
