...
import { Scale, AlertTriangle, CheckCircle2, XCircle, Search, Download } from "lucide-react";
            ...
            <tbody>
              {filtered.map(w => (
                <tr key={w.id} className="border-b border-border last:border-0 hover:bg-surface-2/30 transition-colors">
                  ...
                  <td className="p-3"><p className="font-medium text-text-primary">{w.orderId}</p><p className="text-xs text-text-muted">{w.awb}</p></td>
                  ...
                  <td className="p-3">
                    {w.status === 'Open' ? (
                      <div className="flex gap-1">
                        ...
                        <Button size="sm" variant="outline" className="text-xs h-7 text-danger border-danger/30 hover:bg-danger-light" onClick={() => toast.info(`Dispute ${w.id} escalated`)}>Dispute</Button>
                      </div>
                    ) : (
