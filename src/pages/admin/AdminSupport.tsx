  ...
  return (
    <div className="animate-fade-in-up">
      ...
      {supportTickets.length === 0 ? (
        <EmptyState icon={Headphones} title="No support tickets" description="All tickets have been resolved" />
      ) : (
            ...
            <tbody>
              {supportTickets.map(t => (
                <tr key={t.id} className="border-b border-border last:border-0 hover:bg-surface-2/30">
                  ...
                  <td className="p-3"><Button variant="outline" size="sm" className="text-xs h-7" onClick={() => toast.info(`Viewing ticket ${t.id}`)}>View</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
