...
export default function DropshipperWallet() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const w = await walletService.getWalletSummary();
  ...
  const lastBalance = useMemo(
    () => (transactions[0] ? transactions[0].balance : balance ?? 0),
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div
        className="rounded-xl p-6 mb-6 text-primary-foreground"
        style={{ background: "linear-gradient(135deg, hsl(var(--color-primary)), hsl(var(--color-secondary)))" }}
      >
        <p className="text-sm opacity-80">Available Balance</p>
        <p className="text-3xl font-bold mt-1">₹ {lastBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
        <p className="text-sm opacity-70 mt-1">From latest wallet sync</p>
        <div className="flex gap-3 mt-4">
          <Button
            className="bg-card/20 hover:bg-card/30 text-primary-foreground border-0"
            onClick={() => toast.success("Funds added successfully")}
            type="button"
          >
          ...
          <Button
            variant="outline"
            className="border-card/30 text-primary-foreground hover:bg-card/10"
