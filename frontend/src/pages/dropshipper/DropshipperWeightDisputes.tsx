...
import { useWeightDisputes } from "@/hooks/useApiData";
    ...
    <div className="animate-fade-in-up">
      ...
      <div className="rounded-xl bg-card shadow-card p-5 mb-6 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-sm text-text-muted">Open Disputes</p><p className="text-xl font-bold text-text-primary">{weightDisputes.filter(w => w.status === 'Open').length}</p></div>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={Scale} title="No disputes found" description="No weight disputes match the selected filter" actionLabel="Show All" onAction={() => setTab('all')} />
      ) : (
            ...
            <tbody>
              {filtered.map(w => (
                <tr key={w.id} className="border-b border-border last:border-0 hover:bg-surface-2/30">
                  ...
                  <td className="p-3">
                    {w.status === 'Open' && <Button size="sm" variant="outline" className="text-xs h-7 text-danger border-danger/30 hover:bg-danger-light" onClick={() => toast.success(`Dispute raised for ${w.awb}`)}>Raise Dispute</Button>}
                    {w.status !== 'Open' && <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => toast.info(`Viewing dispute ${w.id}`)}>View</Button>}
                  </td>
                </tr>
              ))}
            </tbody>
