...
export default function AdminPincode() {
  ...
  const { data: pincodeList = [], isLoading } = usePincodes();

  const byPin = useMemo(() => {
    const m = new Map<string, PincodeService>();
    for (const p of pincodeList) m.set(p.pincode, p as PincodeService);
    return m;
  }, [pincodeList]);

  const handleCheck = () => {
    setSearched(true);
    const found = byPin.get(pincode);
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-lg bg-card shadow-card p-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input value={pincode} onChange={(e) => setPincode(e.target.value)} placeholder="e.g. 400001" className="mt-1" maxLength={6} />
            </div>
            <Button
              onClick={handleCheck}
              disabled={isLoading}
          ...
          {isLoading && <p className="mt-4 text-sm text-text-muted">Loading pincodes…</p>}
          ...
          {searched && !isLoading && !result && (
            <div className="mt-4 rounded-lg bg-danger-light p-4 text-center">
              ...
              <p className="text-sm text-text-muted mt-1">This pincode is not in the current serviceability list.</p>
            </div>
          )}
        </div>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-secondary">
                    {result.city}, {result.state} · Zone {result.zone}
                  </p>
                </div>
              </div>
                  ...
                  {result.couriers.map((c) => (
                    <tr key={c.name} className="border-b border-border last:border-0">
                      ...
                      <td className="p-3 text-center">
                        {c.surface ? <CheckCircle2 className="h-4 w-4 text-success mx-auto" /> : <XCircle className="h-4 w-4 text-text-muted mx-auto" />}
                      </td>
                      <td className="p-3 text-center">
                        {c.air ? <CheckCircle2 className="h-4 w-4 text-success mx-auto" /> : <XCircle className="h-4 w-4 text-text-muted mx-auto" />}
                      </td>
                      <td className="p-3 text-center">
                        {c.cod ? <CheckCircle2 className="h-4 w-4 text-success mx-auto" /> : <XCircle className="h-4 w-4 text-text-muted mx-auto" />}
                      </td>
        ...
        <div className="lg:col-span-3 rounded-lg bg-card shadow-card p-4 overflow-x-auto">
          <h3 className="text-sm font-semibold text-text-primary mb-2">Reference: zone rate slabs (₹)</h3>
          <p className="text-xs text-text-muted mb-3">From static rate card — not live pricing</p>
          <table className="w-full text-xs">
            ...
            <tbody>
              {rateCardData.zones.map((z) => (
                <tr key={z} className="border-b border-border/50">
                  <td className="p-2 font-medium">Zone {z}</td>
