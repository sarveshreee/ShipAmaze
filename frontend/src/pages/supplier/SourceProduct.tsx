...
export default function SourceProduct() {
  ...
  const [vendorList, setVendorList] = useState<{ id: string; name: string; city: string; pin: string }[]>([]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setVendorList(Array.isArray(list) ? list : []);
      } catch {
        setVendorList([]);
      }
    })();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = (await productService.getProductById(editId)) as Record<string, unknown>;
        if (data && Object.keys(data).length) hydrate(data);
        const vRows = (await productService.getProductVariants(editId)) as Record<string, unknown>[];
        if (vRows?.length) {
          setVariants(
            vRows.map((v) => ({
  ...
  const onPickImages = async (files: FileList | null) => {
    ...
    try {
      ...
      toast.success(`${urls.length} image${urls.length > 1 ? "s" : ""} added (stored as data in product)`);
    } catch (err: unknown) {
      toast.error(`Failed: ${err instanceof Error ? err.message : "read error"}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const save = async (status: "draft" | "active") => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        countryOfOrigin: form.country_of_origin,
        warranty: form.warranty || undefined,
        manufacturer: form.manufacturer || undefined,
        careInstructions: form.care_instructions || undefined,
        ...
        dimensions: [form.length_cm, form.width_cm, form.height_cm].filter(Boolean).join("x") || undefined,
        vendorName: finalVendorName || undefined,
        uploadedByRole: role,
        variants: variants,
      };
      if (role === "admin" && finalVendorId) {
        payload.vendorId = finalVendorId;
      }
        ...
        {step === "details" && (
          <>
            ...
            <label className={cn("block border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-warning transition-colors", uploading ? "opacity-60 cursor-wait" : "cursor-pointer")}>
              ...
              <p className="text-xs text-text-muted mt-1">PNG, JPG, WEBP — max 5MB each. Images are stored as data URLs on the product.</p>
            </label>
